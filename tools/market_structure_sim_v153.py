...
def run(bars, log=None, box_log=None):
    ...
    # structure state
    trend = 0
    u = dict(ext=None, bar=None, lock=False, pbx=None, pbxBar=None, bnd=None, pend=0, ex=None)
    d = dict(ext=None, bar=None, lock=False, pbx=None, pbxBar=None, bnd=None, pend=0, ex=None)
    key = None
    for t in range(n):
        ev = []
        ...
        if t >= 1:
            ...
            after = (lastClose is None or t - 1 > lastClose) and not ((trend == 1 and u['lock']) or (trend == -1 and d['lock']))   # no range inside a pullback box
        ...
        if active:
            ...
            if realBO:
                ...
                if outU > 0:
                    top = max(top, exH, H[t]); outU = 0; exH = None
                elif C[t] > top:
                    outU = 1; exH = H[t]
                else:
                    top = max(top, H[t])
                if outD > 0:
                    bot = min(bot, exL, L[t]); outD = 0; exL = None
                elif C[t] < bot:
                    outD = 1; exL = L[t]
                else:
                    bot = min(bot, L[t])
        ...
        ctsUp = real_breakout(m, t, u['bnd'] if u['lock'] else None, 1)     # tested against the EXPANDED pullback-box boundary
        ctsDn = real_breakout(m, t, d['bnd'] if d['lock'] else None, -1)
        sig = 0; tp = trend
        if tp == 1 and revDn:
            sig = -2; trend = -1
            ...
            for s, sd, brk, pbflag, ext_px, pb_px in ((u, 1, ctsUp, pbDn, H[t], L[t]), (d, -1, ctsDn, pbUp, L[t], H[t])):
                ...
                if s['lock']:
                    if brk:
                        ...
                        ev.append(('CTS-' + ('up' if sd == 1 else 'dn'), s['ext'], 'BOS', s['pbx'], 'broken box edge', s['bnd']))
                        key = s['pbx']
                        ...
                        # pullback box: the CTS-side edge expands with fake breakouts (two-candle rule)
                        ext_fn = max if sd == 1 else min
                        if s['pend']:
                            s['bnd'] = ext_fn(s['bnd'], s['ex'], ext_px); s['pend'] = 0; s['ex'] = None
                        elif beyond(C[t], s['bnd']):
                            s['pend'] = 1; s['ex'] = ext_px
                        else:
                            s['bnd'] = ext_fn(s['bnd'], ext_px)
                else:
                    ...
                    if pbflag:
                        ...
                        if s['pbx'] is None:
                            s.update(pbx=pb_px, pbxBar=t)
                        s.update(bnd=s['ext'], pend=0, ex=None)
                        ev.append(('PULLBACK', 'locks', s['ext'], 'PB-BOX opens'))
