...
from market_structure_sim import metrics, real_breakout, EPS, P
...
def pb_parts(m, t, d):
    ...
    if d == -1:
        return (bear[t-1] and maru[t-1] and bear[t] and maru[t] and rng[t] >= P['pb_min'] * rng[t-1] - EPS and C[t] < C[t-1],
                bear[t-1] and big[t-1] and bear[t] and rng[t] <= P['pb_r2'] * rng[t-1] + EPS and H[t] < mid,
    ...
    return (bull[t-1] and maru[t-1] and bull[t] and maru[t] and rng[t] >= P['pb_min'] * rng[t-1] - EPS and C[t] > C[t-1],
            bull[t-1] and big[t-1] and bull[t] and rng[t] <= P['pb_r2'] * rng[t-1] + EPS and L[t] > mid,
        ...
        if active:
            ...
            if realBO:
                ...
                if C[t] > top:
                    ...
                    if outU <= R['maxf']:
                        top = max(top, exH if exH is not None else H[t], H[t])
                    outU = 0; exH = None
                if C[t] < bot:
                    outD += 1; exL = L[t] if exL is None else min(exL, L[t])
                else:
                    if outD <= R['maxf']:
                        bot = min(bot, exL if exL is not None else L[t], L[t])
                    outD = 0; exL = None
            if closedNow:
